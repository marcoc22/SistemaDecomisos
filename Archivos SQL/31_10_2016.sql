...
AS
BEGIN
   SELECT contrasena,IdFuncionario INTO P_CONTRASENA,P_IDFUNC      
   FROM UsuarioM 
...
values(PIdFuncionario,Pnombre,Ppuesto);
commit;
end prc_ins_func;
/
show error;
...
values(PIdPolicia,PIdFuncionario,Pnombre);
commit;
end prc_ins_pm;
/
show error;
...
values(PIdObjeto,Pdescripcion,PIdInteresado,PIdDecomiso);
commit;
end prc_ins_obj;
/
show error;
